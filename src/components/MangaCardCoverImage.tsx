...
export default function MangaCardCoverImage({ manga, mangaIndex }: Props) {
  ...
  return (
    <>
      ...
      <div className="[&_button]:[@media(pointer:coarse)]:hidden [&_button]:absolute [&_button]:top-1/2 [&_button]:-translate-y-1/2 [&_button]:rounded-full [&_button]:bg-gray-700/50 [&_button]:text-white [&_button]:p-2 [&_button]:ring-gray-400 [&_button]:active:ring-2">
      ...
      {/* 슬라이드 인디케이터 */}
      <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex gap-2 [&_button]:w-3 [&_button]:h-3 [&_button]:rounded-full [&_button]:bg-gray-300 [&_button]:border [&_button]:border-gray-500 [&_button]:aria-pressed:bg-brand-gradient">
        {Array.from({ length: totalSlides }).map((_, i) => (
          <button
