  ...
  return (
    <aside className={`border-r ${className}`}>
      <div className="grid gap-2 p-2 lg:p-3 lg:gap-3">
        ...
        <LibraryHomeLink isGuest={isGuest} libraryCount={libraries.length} mangaCount={mangaCount} onClick={onClick} />
        {libraries.length > 0 && <div className="h-px bg-zinc-800 my-1" />}
        {libraries.map((library) => (
          <LibraryLink key={library.id} library={library} onClick={onClick} />
        ))}
      </div>
    </aside>
  )
