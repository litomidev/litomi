...
export function PrivateBookmarksView({ currentUserLoginId, attemptedLoginId }: Readonly<Props>) {
  return (
    <div className="flex flex-col items-center justify-center grow px-4 py-16 text-center">
      ...
      <div className="max-w-md space-y-3 text-zinc-400 mb-8">
        <p>
          현재 <span className="font-mono text-zinc-300">@{attemptedLoginId}</span>님의 북마크는 본인만 볼 수 있어요
        </p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3">
        <Link
          ...
          href={`/@${currentUserLoginId}/bookmark`}
        >
        ...
        <Link
          ...
          href={`/@${attemptedLoginId}`}
        >
          @{attemptedLoginId}님의 프로필 보기
        </Link>
      </div>
    </div>
  )
